...
SETUP (Replit Secrets — never hardcode real values):
  ...
  POLYMARKET_WALLET   → your Polymarket wallet address
...
SLASH_GOLF_KEY     = os.environ.get("SLASH_GOLF_KEY", "YOUR_SLASH_GOLF_KEY_HERE")
...
POLYMARKET_API_KEY = os.environ.get("POLYMARKET_API_KEY", "YOUR_POLYMARKET_API_KEY_HERE")
POLYMARKET_PK      = os.environ.get("POLYMARKET_PK", "YOUR_POLYMARKET_PK_HERE")
POLYMARKET_WALLET  = os.environ.get("POLYMARKET_WALLET", "YOUR_WALLET_HERE")
...
state = {
    ...
    "open_positions": [],       # list of dicts: {player, market_id, side, size, entry_pct, order_id}
...
def fetch_polymarket_odds(player_names: list) -> dict:
    ...
    Real implementation requires Polymarket CLOB market search.
    """
    try:
...
def place_polymarket_order(market_id: str, side: str, size_usd: float, player_name: str) -> dict:
    """
...
def exit_polymarket_position(market_id: str, size_usd: float, player_name: str) -> dict:
...
def run_preflight(player: str, edge: float, confidence: str, round_num: int) -> tuple[bool, str]:
...
def execute_trade(player: str, market_id: str, edge: float, confidence: str,
    ...
    result = place_polymarket_order(market_id, "YES", size, player)
...
def run_profit_cycle(round_num: int):
    ...
    realized = sum(
        p.get("pnl", 0) for p in state["closed_positions"]
    )
    if realized <= 0:
...
--------------------""")
    ...
    # Check for no-growth suspension
...
def run_scan(round_num: int, trigger: str = "SCHEDULED", from_cycle: bool = False):
    ...
    # Parse signals and execute (simplified parser — Claude returns structured blocks)
    ...
    for line in lines:
        if line.startswith("PLAYER:"):
            ...
            # All fields collected — attempt trade if ENTER
            if current_player.get("signal") == "ENTER":
                ...
                if player and player not in state["cut_players"]:
                    ...
                    # Market ID lookup would happen here — placeholder
                    market_id = f"USOPEN2026_{player.replace(' ', '_').upper()}"
                    execute_trade(player, market_id, edge, confidence, entry_pct, round_num, from_cycle)
            current_player = {}
...
def handle_command(text: str):
    ...
    if cmd == "AUTOPILOT: ON":
        ...
        if pos:
            ...
            result = exit_polymarket_position(pos["market_id"], half_size, player)
        ...
        pos = next((p for p in state["open_positions"] if p["player"].lower() == player.lower()), None)
        if pos:
            result = exit_polymarket_position(pos["market_id"], pos["size"], player)
...
def schedule_loop():
    ...
    while True:
        ...
        if round_num == 1:
            ...
            # Trigger R1 monitor when all groups finished (fog delay — expect ~01:00 UTC = 9pm ET)
...
-------------------------""")
        ...
        # ── FRIDAY R2 ──
            ...
            # 18:00 UTC = 2pm ET → send bankroll reminder before R2 trading window
            ...
            # R2 Scan 1 — when all R2 groups finished
            ...
            # Live movement triggers during R2
            check_movement_triggers(2)

        # ── SATURDAY R3 ──
        elif round_num == 3:
            # 08:00 UTC = 4am ET... adjust: 12:00 UTC = 8am ET
            ...
            # Saturday night R4 preview — 01:00 UTC = 9pm ET
            if hour == 1 and minute < 5 and last_r3_scan2 and not last_r3_night:
                ...
                # Exit players outside 4-shot gate
                lb = fetch_leaderboard()
                ...
                for pos in state["open_positions"][:]:
                    ...
                    if shots_back > 4:
                        exit_polymarket_position(pos["market_id"], pos["size"], pos["player"])
                        state["open_positions"].remove(pos)
...
if __name__ == "__main__":
    print("Starting DAEMON-POLY Bot...")
