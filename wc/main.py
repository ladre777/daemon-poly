...
def main():
    ...
    # prior boots, and a fresh-only rebuild could never repair that.
    # Idempotent: slugs already tracked in state are skipped.
    try:
        ...
        if _recovered_pos:
            ...
            if _n_added:
                print(f"State recovery: added {_n_added} position(s) from Polymarket into state")
                try:
                    send_status(
                        f"\U0001f504 State recovery: found {_n_added} real Polymarket "
                        f"position(s) missing from state \u2014 added with "
            ...
            print("State recovery: no open Polymarket positions \u2014 nothing to merge")
    except Exception as _fserr:
        print(f"State recovery query failed (non-fatal): {_fserr}")
        try:
            send_error(
                f"\u26a0\ufe0f Startup PM position recovery failed: {_fserr} \u2014 "
